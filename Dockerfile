...
RUN apk update && apk upgrade --no-cache && \
    ...
    make CXXFLAGS="-O3 -DNDEBUG -w -mtune=native -flto=$(nproc) -pipe" HAVE_RULES=yes FILESDIR=/usr/share/cppcheck MATCHCOMPILER=yes -j $(nproc) install && \
    cd ../.. && \
    rm -rf cppcheck && rm -rf patches && \
    apk del --purge --no-cache git g++ libstdc++-dev make pcre-dev python3 && \
    rm -rf /var/cache/apk/*
